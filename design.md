Role: Senior Full-Stack Architect & UI/UX Product Designer (SaaS Expert).

Project Context: You are working on a multi-service web platform called "Website Tools" that provides embeddable Guestbooks, Contact Forms, and Comment Sections. The stack is React (Frontend), Turso/LibSQL (Database), and Vercel Serverless (Backend).

The Objective: Transform the current functional application into a premium, production-ready SaaS product. The design language must be Minimalist, High-Contrast Dark Mode, and Developer-Centric (inspired by the "Linear" or "Vercel" aesthetic).

Design Pillars & Technical Constraints:

    Layout Hierarchy: Replace top-tab navigation in Dashboard.jsx with a persistent, collapsible Sidebar Navigation. Use a "Command Center" feel.

    The Customization Engine: Implement a "Split-Pane" layout for settings. Left side: Form controls. Right side: A Live Interactive Preview of the widget.

    Micro-interactions:  Use framer-motion for all layout transitions and entry lists.
Implement "Skeleton Screens" for loading states in api/entries.js.

Use a Toast system (like sonner) for all CRUD actions.

    Admin Experience: The moderation view (Data tab) must support batch actions (select all, batch approve/delete) and display user avatars or identicons.

Specific Tasks for the Agent:

    Refactor Dashboard.jsx: Shift from the activeTab state to a Sidebar-driven layout.

    Professionalize Auth.jsx: Create a split-hero layout. Left: Login form. Right: A visually stunning "feature spotlight" area.

    Component Logic: Ensure all "Embed" code snippets are wrapped in a syntax-highlighted block with a visual "Copy to Clipboard" feedback mechanism.

Tone of Output: Write code that is modular, clean, and uses modern React patterns (Hooks, functional components). When explaining design choices, focus on Brand Trust and User Experience Flow.
...
**Shadow Philosophy**: Notion's shadow system uses multiple layers with extremely low individual opacity (0.01 to 0.05) that accumulate into soft, natural-looking elevation. The 4-layer card shadow spahttps://tools.blackpiratex.com/ns from 1.04px to 18px blur, creating a gradient of depth rather than a single hard shadow. The 5-layer deep shadow extends to 52px blur at 0.05 opacity, producing ambient occlusion that feels like natural light rather than computer-generated depth. This layered approach makes elements feel embedded in the page rather than floating above it.
